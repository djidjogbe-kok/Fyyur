...
def show_artist(artist_id):
  ...
  for show in past_shows:
    last={}
    last["venue_id"]=show.venues.id
    last["venue_name"]=show.venues.name
    last["venue_image_link"]=show.venues.image_link
  ...
  #upcoming shows for the artist
  upcoming_shows=list(filter(lambda show:show.start_time>datetime.now(), artist.shows))
  next_shows=[]
  for show in upcoming_shows:
    next={}
    next["venue_id"]=show.venues.id
    next["venue_name"]=show.venues.name
    next["venue_image"]=show.venues.image_link
    next["start_time"]=show.venues.strftime("%m/%d/%Y, %H:%M:%S")
    next_shows.append(next)
...
def edit_venue_submission(venue_id):
  ...
  if form.validate():
    try:
      ...
      #insert edited data to the venue table
      ...
      db.session.rollback()
      print(sys.exc_info())
      flash("Venue wasn't edited with success")
    finally:
      db.session.close()
  else:
    print("\n\n", form.errors)
...
def shows():
  ...
  for show in shows:
    hold={}
    hold["venue_id"]=show.venues.id
    hold["venue_name"]=show.venues.name
    hold["artist_id"]=show.artists.id
    hold["artist_name"]=show.artists.name
    hold["artist_image_link"]=show.artists.image_link
    hold["start_time"]=show.start_time.strftime("%m/%d/%Y, %H:%M:%S")
    data.append(hold)
  ''' data=[{
    "venue_id": 1,
    "venue_name": "The Musical Hop",
...
def create_show_submission():
  ...
  if form.validate():
    ...
    try:
      new_show =Show(artists_id=form.artist_id.data,
  ...
  # on successful db insert, flash success
      ...
      db.session.rollback()
      print(sys.exc_info())
      flash('An error occurred. Show could not be listed. Make sure the IDs are the correct ones.')
    # TODO: modify data to be the data object returned from db insertion
    finally:
      db.session.close()
  else:
